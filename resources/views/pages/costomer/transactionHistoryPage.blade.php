    ...
    <div class="space-y-4 min-h-[calc(100vh-92px)] p-6">
        @if($transactionHistory->isEmpty())
            <div class="bg-gray-800/60 shadow rounded-lg p-6">
                <p class="text-gray-500 dark:text-gray-400">No transaction history found for your account.</p>
            </div>
        @else
                    ...
                    <tbody>
                        @foreach($transactionHistory as $transaction)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-4 py-2 border-b">Rs. {{ number_format($transaction->amount, 2) }}</td>
                                <td class="px-4 py-2 border-b">{{ $transaction->salesDateTime }}</td>
                                <td class="px-4 py-2 border-b">{{ $transaction->shippingMethod }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
