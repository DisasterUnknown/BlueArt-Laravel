<x-guest-layout>
    <div class="bg-cover bg-center min-h-screen" style="background-image: url('/assets/LoginBackground.gif')">
        ...
        <div class="flex justify-between absolute top-0 left-0 w-full">
            <a href=""></a>
            <a href="{{ url('/aboutUs') }}"
                class="border bg-white bg-opacity-5 backdrop-blur-lg mt-2 mr-2 py-0.5 pb-1 px-3 text-white font-semibold rounded">
                About Us
            </a>
        </div>
        ...
        <div class="flex items-center justify-center min-h-screen">
            <form method="POST" action="{{ route('login') }}"
                class="bg-white bg-opacity-5 backdrop-blur-lg p-8 w-[85%] sm:w-[350px] shadow-lg rounded">
                @csrf
                ...
                <div class="flex flex-col justify-center mb-2">
                    ...
                    <input id="email"
                        class="border border-white bg-white text-white bg-opacity-5 px-3 mt-1 py-0.5 w-full rounded-full hover:bg-opacity-50 focus:outline-none focus:ring-2 focus:ring-pink-400"
                ...
                <div class="flex items-center justify-center mt-2 mb-4">
                    <div class="flex items-center justify-end mt-4">
                        @if (Route::has('password.request'))
                            <a class="text-sm text-pink-200 hover:text-white underline"
                ...
                @if ($errors->any())
                    <div class="bg-red-100/20 border border-red-400/20 text-red-700 mt-5 mb-5 px-4 py-3 rounded">
                        @foreach ($errors->all() as $error)
                            ...
                            {{-- Redirect if banned --}}
                            @if (str_contains($error, 'suspended') || str_contains($error, 'banned'))
                                <script>
                                    window.location.href = "{{ route('requestUnban') }}";
                                </script>
                            @endif
                        @endforeach
                    </div>
